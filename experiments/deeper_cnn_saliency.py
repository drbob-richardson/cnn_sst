...
os.makedirs(str(_Path(__file__).resolve().parents[1] / "figures" / "saliency_deeper_cnn"), exist_ok=True)
...
for lead_time in lead_times:
    ...
    save_dir = os.path.join(str(_Path(__file__).resolve().parents[1] / "figures" / "saliency_deeper_cnn"), f"lead_{lead_time}")
    os.makedirs(save_dir, exist_ok=True)
    compute_and_save_saliency(model, data_res1, labels, save_dir, lead_time, device)

# Save results to a CSV file
results_df = pd.DataFrame(results)
results_df.to_csv(str(_Path(__file__).resolve().parents[1] / "results" / "deeper_cnn_results.csv"), index=False)
