...
from src.data import process_data_multi_res
...
os.makedirs(str(_Path(__file__).resolve().parents[1] / "figures" / "saliency_mixed_res"), exist_ok=True)
...
for lead_time in lead_times:
    for k in resolutions:
        ...
        save_dir = os.path.join(str(_Path(__file__).resolve().parents[1] / "figures" / "saliency_mixed_res"), f"lead_{lead_time}_res_{k}")
...
results_df.to_csv(str(_Path(__file__).resolve().parents[1] / "results" / "mixed_resolution_results.csv"), index=False)
